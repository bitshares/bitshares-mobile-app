...
class OpenLedger : GatewayBase() {
    ...
    override fun queryCoinList() : Promise {
        ...
        val api_base = _api_config_json.optString("base")
    ...
    override fun processCoinListData(data_array: JSONArray, balanceHash: JSONObject): JSONArray? {
        ...
        data_assets.forEach<JSONObject> { it ->
            ...
            val enableWithdraw = withdraw_options.getBoolean("healthy") && withdraw_options.getInt("status") != 0
            val enableDeposit = deposit_options.getBoolean("healthy") && deposit_options.getInt("status") != 0
            ...
            val deposit_amount = deposit_item!!.getJSONObject("amount").getJSONObject("destination");
            ...
            var balance_item = balanceHash.optJSONObject(symbol) ?: JSONObject().apply{ put("iszero", true) }

            val backingCoin = withdraw_item!!.optJSONObject("destination").getString("asset").toUpperCase()
            ...
            appext.supportMemo = withdraw_item.getJSONObject("memo").getJSONObject("destination").getBoolean("enabled")
            ...
            val new_item = JSONObject(item.toString())
            new_item.put("kAppExt",appext)
            result.put(JSONObject(new_item.toString()))
        }
    ...
    override fun requestDepositAddress(item: JSONObject, fullAccountData: JSONObject, ctx: Context) : Promise {
        ...
        val outputAddress = account_data.getJSONObject("name")
