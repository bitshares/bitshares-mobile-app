...
//    detailsLabel.textColor = [UIColor whiteColor];
    //  REMARK: 不添加参数，直接在这里修改了。
    detailsLabel.textColor = [ThemeManager sharedThemeManager].textColorMain;
...
- (void)layoutSubviews {
    ...
    // extraView
    if (extraView && extraView.bounds.size.height > 0.f){
        totalSize.height += kPadding;
        totalSize.height += extraView.bounds.size.height;
    }
    ...
    // extraView
    if (extraView && extraView.bounds.size.height > 0.f){
        yPos += kPadding + detailsLabelF.size.height;
        CGRect extraViewF = extraView.frame;
        CGSize extraViewSize = extraViewF.size;
        extraViewF.origin.y = yPos;
        extraViewF.origin.x = roundf((bounds.size.width - extraViewSize.width) / 2) + xPos;
        extraViewF.size = extraViewSize;
        extraView.frame = extraViewF;
    }
